def find_best_flights(flights, origin, destination):
    ...
    for x in flights:
        if x[1] not in g:
            g[x[1]] = {}
        g[x[1]][(x[2], x[0])] = [x[3], x[4], x[5]]
        #  g[origin][(destination, flight number)] = [in, out, cost]

    path = {}
    for x in g[origin]:
        path[x] = g[origin][x]+[[x[1]]]
    #  path[(destination, flight number)] = [in, out, cost, PATHS]
    marked = []
    while 1:
        m = 100000000000000
        current = []
        for x in path:
            if x not in marked:
                if path[x][2] < m:
                    m = path[x][2]
                    current = x  # (destination, flight number)
        marked.append(current)
        if len(current) == 0:
            break
        for neigh in g[current[0]]:
            if int(path[current][1][0:2])*60+int(path[current][1][3:]) < int(g[current[0]][neigh][0][0:2])*60+int(g[current[0]][neigh][0][3:]):
                if neigh not in marked:
                    if neigh in path:
                        if path[neigh][2] > path[current][2]+g[current[0]][neigh][2]:
                            path[neigh][2] = path[current][2]+g[current][neigh][2]
                            path[neigh][0] = path[current][0]
                            path[neigh][3] = path[current][3] + [neigh[1]]
                        elif path[neigh][2] == path[current][2]+g[current[0]][neigh][2] and (int(path[current][1][0:2])*60+int(path[current][1][3:])-int(path[neigh][1][0:2])*60-int(path[neigh][1][3:])) < 0:
                                path[neigh][0] = path[current][0]
                                path[neigh][3] = path[current][3] + [neigh[1]]
                    else:
                        cost = g[current[0]][neigh][2]+path[current][2]
                        paths = path[current][3] + [neigh[1]]
                        path[neigh] = [path[current][0], g[current[0]][neigh][1], cost, paths]
    s = [path[x] for x in path if x[0] == destination]
    if len(s)==0:
        return None
    m = min([x[2] for x in s])
    result = [x[3] for x in s if x[2] == m]
    time = [int(x[1][0:2])*60+int(x[1][3:])-int(x[0][0:2])*60-int(x[0][3:]) for x in s if x[2] == m]
    print(result, time)
    return result[time.index(min(time))]
