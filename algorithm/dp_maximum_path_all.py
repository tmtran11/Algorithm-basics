...
# with the weight calculated by the highest possible sum of the money make by th efilms tha connect them.
# running time O(n^3)
...
# actor and all film
r = {}  
# film and all actor and their films
for x in lines:
    t = x[:-1].split('\t')
    if t[0] not in a:
        a[t[0]] = []
...
# a graph with actor the actors they connect to, weight included

nodes = list(a.keys())
table = []
# table element: sum max weight. Using dynamic programming
